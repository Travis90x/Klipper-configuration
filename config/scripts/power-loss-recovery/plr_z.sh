...
cp /home/pi/printer_data/gcodes/.thumbs/${4}.png /home/pi/printer_data/gcodes/.thumbs/plr.png
cp /home/pi/printer_data/gcodes/.thumbs/${4}-32x32.png /home/pi/printer_data/gcodes/.thumbs/plr-32x32.png
cp /home/pi/printer_data/gcodes/.thumbs/${4}-400x400.png /home/pi/printer_data/gcodes/.thumbs/plr-400x400.png



# clean old temp files
rm /tmp/plrtmpA.* > /dev/null 2>&1
...
if [ $isInFile -eq 0 ]; then
     echo 'M109 S240.0' > ${SD_PATH}/plr.gcode
...
echo '; z_height = '${1} >> ${SD_PATH}/plr.gcode
echo '; last_file = '${2} >> ${SD_PATH}/plr.gcode
echo '; print_temp = '${3} >> ${SD_PATH}/plr.gcode
echo '; thumbnail = '${4} >> ${SD_PATH}/plr.gcode

echo 'SET_KINEMATIC_POSITION Z=0' >> ${SD_PATH}/plr.gcode
echo 'G91' >> ${SD_PATH}/plr.gcode
echo 'G1 Z1' >> ${SD_PATH}/plr.gcode
echo 'G90' >> ${SD_PATH}/plr.gcode
echo 'M84' >> ${SD_PATH}/plr.gcode
echo 'G28' >> ${SD_PATH}/plr.gcode
...
# cat /tmp/plrtmpA.$$ | sed -ne '/;End of Gcode/,$ p' | tr '\n' ' ' | sed -ne 's/ ;[^ ]* //gp' | sed -ne 's/\\\\n/;/gp' | tr ';' '\n' | grep nmaterial_print_temperature | sed -ne 's/.* = /M109 S/p' | head -1 >> ${SD_PATH}/plr.gcode
# cat /tmp/plrtmpA.$$ | sed -ne '/;End of Gcode/,$ p' | tr '\n' ' ' | sed -ne 's/ ;[^ ]* //gp' | sed -ne 's/\\\\n/;/gp' | tr ';' '\n' | grep material_bed_temperature | sed -ne 's/.* = /M190 S/p' | head -1 >> ${SD_PATH}/plr.gcode


# Set Position for Extruder:  G92 EXXX at first Z(z_height)
...
tac /tmp/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]* /q' | tac | sed -ne '/ Z/,$ p' | sed -e '/ E[0-9]/ q' | sed -ne 's/.* E\([^ ]*\)/G92 E\1/p' >> ${SD_PATH}/plr.gcode
...
tac /tmp/plrtmpA.$$ | sed -e '/ Z'${1}'[^0-9]* /q' | tac | tail -n+1| sed -ne '/ Z/,$ p' >> ${SD_PATH}/plr.gcode
